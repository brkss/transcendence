		...
		<Box pos={'absolute'}  mt={'10px'} w={'100%'} bg={'white'} p={'10px'}  rounded={'7px'}>
			{
				results.map((user, key) => (
					<Box key={key} display={'flex'} color={'black'} padding={'10px'} alignItems={'center'} _hover={{bg: '#eae5e5', transition: '.3s'}} transition={'.3s'} rounded={'5px'} mb={'10px'} cursor={'pointer'}>
						<Avatar d={'40px'} />
						<Box>
							<Text marginLeft={'15px'} fontWeight={'bold'}>Brahim Berkasse</Text>
							<Text fontSize={'13px'} opacity={'.8'} marginLeft={'15px'} fontWeight={'bold'}>@bberkass</Text>
						</Box>
					</Box>
				))
			}
