...
interface Props {
	isProctected?: boolean;
	name: string;
	enter: () => void;
}

export const ChatBox: React.FC<Props> = ({isProctected, name, enter}) => {
		...
		<Box mt={'20px'} onClick={enter}>
			<Stack direction={['column', 'row']} spacing={'10px'} cursor={'pointer'} transition={'.3s'} _hover={{bg: 'blackAlpha.100', transition: '.3s'}} p={'10px'} rounded={'10px'}>
				...
				<Box>
					...
					{ isProctected && <Text bg={'green.100'} rounded={'4px'} fontWeight={'bold'} fontSize={'13px'} p={'1px 6px'} display={'inline-block'}>Password protected</Text>}
				</Box>
				<Box ml={'auto'}>
