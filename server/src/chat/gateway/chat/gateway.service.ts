import {
  ...
  export class GatewayService {
    ...
    private async joinPrevRooms(
      socket: Socket,
      userId: number,
    ) {
      const all_rooms =
        await this.roomService.getUserJoinedRooms(
          userId,
        );
      if (
        all_rooms ===
    ...
    async leavAllSocketRooms(
      ...
      await this.roomService.deleteMemberFromRooms(
        user.id,
      );
